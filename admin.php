                    ...
                    <?php foreach (getAllUsers() as $user) : ?>
                        <tr>
                            ...
                            <td><?php if (isOwner($user['id'])) : ?>
                              ...
                              <?php else : ?>
                                <?php if ($user['role_id'] == 1 && $user['username'] != 'tmar') : ?>
                                        <a href="procedures/adjustRole.php?roleId=2&userId=<?php echo $user['id']; ?>"
                                           class="btn btn-xs btn-warning">Demote
                                            to General User</a>
                                <?php elseif ($user['role_id'] == 2): ?>
                                    <a href="procedures/adjustRole.php?roleId=1&userId=<?php echo $user['id']; ?>"
                                       class="btn btn-xs btn-success">Promote
                                            to Admin User</a><?php endif; ?><?php endif; ?></td>
                            <td><?php if ($user['username'] != 'tmar'): ?>
                                    <a href="procedures/adjustRole.php?roleId=3&userId=<?php echo $user['id']; ?>"
